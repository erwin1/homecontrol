...
import evcharging.impl.homewizard.HWEP1;
import evcharging.impl.sma.SMAAuthException;
import evcharging.impl.sma.SMACharger;
import evcharging.impl.sma.SMAInverter;
import evcharging.impl.sma.SMAMeterData;
...
public class MetricsService {
    ...
    public LiveMetrics getLiveMetrics() {
        SMAMeterData values = inverter.getLiveSMAPowerData();
        long pvW = values.getFromPV();
        long gridInOutW = values.getToGrid() > 0 ? -values.getToGrid() : values.getFromGrid();
        long chargerW = 0;
        try {
            chargerW = charger.getLivePowerMeterReading();
        } catch (SMAAuthException e) {
            LOGGER.severe("could not get live power meter reading from charger "+e);
        }
        ...
        metrics.setPvW(new BigDecimal(pvW));
        metrics.setEvBatteryLevel(ev.getCurrentBatteryLevel());
        metrics.setEvConnected(!charger.getState().equals(EVCharger.State.NotConnected));//TODO use cached value here
        return metrics;
    }
